...
`MASTER-006` wave 4 is now active. The search layer is kept separate from canonical knowledge and currently contains **540 reviewed non-canonical queries**: 406 EN/JA direct-definition queries covering all **203 P0 Concepts**, 40 EN/JA comparison queries covering all **20 current reviewed Concept relations**, 28 EN/JA regional-usage queries covering **14 curated source-verified regional label/use cases**, plus 33 abbreviation lookups and 33 racecard-reading lookups. How/why and translation remain unseeded; regional usage and lower-priority definitions are intentionally not blanket-generated.
...
`GLOSSARY-MASTER-006` is now **in progress**. Waves 1-4 add the search-intent policy, audited query registry, complete P0 EN/JA definition coverage, EN/JA comparison coverage for all current reviewed Concept relations, and a curated regional-usage seed grounded in source-verified regional labels. The current decision remains **HOLD** until how/why/translation coverage and worldwide domain/jurisdiction/language readiness audits are completed. Public glossary architecture remains deferred.
