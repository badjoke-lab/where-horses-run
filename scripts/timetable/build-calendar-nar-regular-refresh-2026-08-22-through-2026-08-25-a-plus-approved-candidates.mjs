...
const REVIEWED_AT = '2026-08-22T14:40:00Z';
const REVIEWER = 'badjoke-lab';
const SOURCE_BATCH_ID = 'due-job-plan-2026-08-21-due-japan-nar-regular-refresh-001-run-001';
const SOURCE_GENERATED_AT = '2026-08-21T04:38:58.183Z';
const SOURCE_BLOB_SHA = '89410debbd648e1264933df37b0ef246945a1873';
const SOURCE_START_DATE = '2026-08-22';
const SOURCE_END_DATE_EXCLUSIVE = '2026-09-05';
...
assert((batch.scheduled_meetings ?? []).length === 47, 'NAR scheduled meeting count differs');
assert((batch.detail_candidates ?? []).length === 6, 'NAR complete detail candidate count differs');
assert((batch.schedule_candidates ?? []).length === 41, 'NAR schedule-only candidate count differs');
assert((batch.detail_blockers ?? []).length === 41, 'NAR detail blocker count differs');
...
assert(JSON.stringify(actualIds) === JSON.stringify(EXPECTED_IDS), `reviewed A+ identity set differs: ${JSON.stringify(actualIds)}`);
...
const records = detailCandidates.map((candidate) => {
  ...
  const record = {
    ...
    notes: 'Approved from the pinned NAR regular-refresh complete detail candidate set. Only meeting identity and the six public-safe A+ timetable fields are promoted.',
  };
...
assert(records[0].date === '2026-08-22', 'first approved date differs');
assert(records.at(-1).date === '2026-08-25', 'last approved date differs');
assert(records.reduce((sum, record) => sum + record.timetable_rows.length, 0) === 58, 'approved race-row total differs');

const output = {
  schema_version: 'timetable-candidate-v1',
  generated_at: batch.generated_at,
  adapter_id: 'nar-regular-refresh-2026-08-22-through-2026-08-25-reviewed-detail-promotion-v1',
  country_id: 'japan',
  ...
  review: {
    ...
    reviewer: REVIEWER,
    summary: 'Approved exactly six complete A+ NAR detail records from the pinned August 21 regular-refresh batch; the remaining 41 schedule-only records are not approved by this envelope.',
