...
const MODES = new Set(['date_window', 'source_visible_horizon', 'selected_meetings']);
...
export function buildActionsCollectionPlan(env = process.env) {
  ...
  assert(MODES.has(mode), 'WHR_MODE must be date_window, source_visible_horizon, or selected_meetings.');

  const args = [`--batch-id=${batchId}`];

  if (mode === 'date_window' || mode === 'source_visible_horizon') {
    assert(isRealDate(startDate), `WHR_START_DATE must be a real YYYY-MM-DD date for ${mode} mode.`);
    assert(isRealDate(endDateExclusive), `WHR_END_DATE_EXCLUSIVE must be a real YYYY-MM-DD date for ${mode} mode.`);
    assert(startDate < endDateExclusive, 'WHR_END_DATE_EXCLUSIVE must be after WHR_START_DATE.');
    assert(meetingIds.length === 0, `WHR_MEETING_IDS must be empty for ${mode} mode.`);
