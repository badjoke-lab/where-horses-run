...
import {
  ...
  buildSorocabaMeetingRecord,parseSorocabaCalendarText,
...
async function fetchPdfText(url){
  ...
  const texts=[];
  for(let n=1;n<=pdf.numPages;n++){const page=await pdf.getPage(n);const content=await page.getTextContent();texts.push(content.items.filter(i=>'str'in i).map(i=>i.str).join(' '));}
  return texts.join(' ');
}
...
try{
  const text=await fetchPdfText(BRAZIL_SOROCABA_CALENDAR_URL);
  allRows=parseSorocabaCalendarText(text,{year:Number(start.slice(0,4)),sourceUrl:BRAZIL_SOROCABA_CALENDAR_URL});
