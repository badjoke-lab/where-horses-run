...
const OUTPUT = 'data/candidates/nar-regular-refresh-2026-08-27-through-2026-08-28-a-plus-approved.json';
const REVIEWED_AT = '2026-08-24T16:05:00Z';
const REVIEWER = 'badjoke-lab';
const SOURCE_BATCH_ID = 'due-job-plan-2026-08-24-due-japan-nar-regular-refresh-001-run-001';
const SOURCE_GENERATED_AT = '2026-08-24T08:10:05.789Z';
const SOURCE_BLOB_SHA = 'a90fcc64d1fadf42a44dedbf0c53f6dd486c063c';
const SOURCE_START_DATE = '2026-08-25';
const SOURCE_END_DATE_EXCLUSIVE = '2026-09-08';
...
assert((batch.schedule_candidates ?? []).length === 35, 'NAR schedule-only candidate count differs');
assert((batch.detail_blockers ?? []).length === 35, 'NAR detail blocker count differs');
...
for (const id of ALREADY_REVIEWED_IDS) {
  const sourceCandidate = candidatesById.get(id);
  assert(sourceCandidate, `${id} source candidate missing`);
  const rows = publicRows(sourceCandidate);
  const meeting = canonicalMeetingById.get(id);
  const detail = canonicalDetailById.get(id);
  assert(meeting?.capability_rank === 'A+', `${id} is not already canonical A+`);
  assert(detail?.capability_rank === 'A+', `${id} is not already canonical A+ detail`);
  assert(meeting.first_race_time_local === rows[0].post_time_local, `${id} already-reviewed first race time changed`);
  assert(meeting.last_race_time_local === rows.at(-1).post_time_local, `${id} already-reviewed last race time changed`);
  assert(JSON.stringify(canonicalPublicRows(detail)) === JSON.stringify(rows), `${id} already-reviewed public-safe timetable fields changed`);
}

const records = APPROVED_IDS.map((id) => {
  const candidate = candidatesById.get(id);
  assert(candidate, `${id} newly reviewed source candidate missing`);
  const expected = EXPECTED_APPROVED.get(id);
  ...
  assert(!currentDetail, `${id} unexpectedly already has canonical detail; review delta must be recalculated`);
  ...
  const record = {
    ...
    notes: 'Approved from the pinned August 24 NAR regular-refresh complete detail candidate set. Only meeting identity and the six public-safe A+ timetable fields are promoted.',
  };
  const serialized = JSON.stringify(record).toLowerCase();
  for (const forbidden of ['horse_name', 'jockey', 'trainer', 'odds', 'payout', 'result', 'prediction', 'raw_html', 'stream_url']) {
    assert(!serialized.includes(forbidden), `${id} reviewed record contains forbidden field ${forbidden}`);
  }
  return record;
});
...
const output = {
  ...
  adapter_id: 'nar-regular-refresh-2026-08-27-through-2026-08-28-reviewed-detail-promotion-v1',
  country_id: 'japan',
  ...
  review: {
    ...
    reviewer: REVIEWER,
    summary: 'Approved exactly four newly complete A+ NAR detail records from the pinned August 24 regular-refresh batch after verifying the other nine complete records are byte-equivalent on all public-safe timetable fields to current canonical A+ detail. The remaining 35 schedule-only records stay unapproved.',
...
console.log(JSON.stringify({
  ...
  verified_noop_records: ALREADY_REVIEWED_IDS.length,
