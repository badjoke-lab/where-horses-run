...
const OUTPUT = 'data/candidates/nar-horizon-extension-2026-09-18-approved.json';
const REVIEWED_AT = '2026-08-20T04:42:49Z';
const REVIEWER = 'badjoke-lab';
const START_DATE = '2026-09-18';
const END_DATE_EXCLUSIVE = '2026-09-19';
...
assert(batch.batch_id === 'due-job-plan-2026-08-20-due-japan-nar-coverage-gap-001-run-001', 'NAR review batch identity differs');
assert(batch.generated_at === '2026-08-20T04:02:57.988Z', 'NAR review batch timestamp differs');
...
assert(batch.requested_scope?.start_date === '2026-09-16', 'NAR requested start date differs');
...
assert(Array.isArray(batch.scheduled_meetings) && batch.scheduled_meetings.length === 11, 'NAR scheduled meeting count differs');
...
assert(records.length === 3, 'reviewed extension must contain exactly three meetings');
assert(records[0].date === START_DATE, `extension first date differs: ${records[0].date}`);
assert(records.at(-1).date === START_DATE, `extension final date differs: ${records.at(-1).date}`);
...
const expectedIds = [
  ...
  'nar-oi-racecourse-2026-09-18',
  'nar-sonoda-racecourse-2026-09-18',
];
...
const output = {
  ...
  adapter_id: 'nar-september-2026-horizon-extension-2026-09-18-reviewed-schedule-promotion-v1',
  country_id: 'japan',
  authority_id: 'nar-local-government-racing',
  source_id: 'nar-monthly-schedule-grid',
  candidate_window: {
