...
export const SOREC_ADAPTER_ID = 'sorec-programme-reunion-v1';
...
export function buildSorecProgrammeCandidate({ html, checkedAt, startDate, endDateExclusive }) {
  ...
  const records = parsed.records.map((row) => {
    ...
    const record = {
      ...
      notes: `Official SOREC Programme Réunion observation; source venue label: ${row.venue_label}.`,
    };
    return {
      ...record,
  ...
  return {
    candidate: {
      ...
      review: {
        ...
        reviewed_at: null,
        reviewer: null,
        summary: 'SOREC observations use centrally derived best-available rank; current index rows contain meeting identity unless richer normalized evidence is present.',
