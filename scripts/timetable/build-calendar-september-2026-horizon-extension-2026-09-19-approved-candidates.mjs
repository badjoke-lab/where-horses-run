...
const OUTPUT = 'data/candidates/nar-horizon-extension-2026-09-19-approved.json';
const REVIEWED_AT = '2026-08-21T04:50:00Z';
const REVIEWER = 'badjoke-lab';
const START_DATE = '2026-09-19';
const END_DATE_EXCLUSIVE = '2026-09-20';
...
assert(batch.batch_id === 'due-job-plan-2026-08-21-due-japan-nar-coverage-gap-001-run-001', 'NAR review batch identity differs');
assert(batch.generated_at === '2026-08-21T04:39:00.732Z', 'NAR review batch timestamp differs');
...
assert(Array.isArray(batch.scheduled_meetings) && batch.scheduled_meetings.length === 1, 'NAR scheduled meeting count differs');
assert(Array.isArray(batch.schedule_candidates) && batch.schedule_candidates.length === 1, 'NAR schedule candidate count differs');
assert(Array.isArray(batch.detail_candidates) && batch.detail_candidates.length === 0, 'NAR detail candidates must remain empty');
assert(Array.isArray(batch.detail_blockers) && batch.detail_blockers.length === 1, 'NAR detail blocker count differs');
assert(batch.detail_blockers[0]?.meeting_id === 'nar-saga-racecourse-2026-09-19', 'NAR detail blocker meeting differs');
assert(batch.detail_blockers[0]?.status === 'parser_failure', 'NAR detail blocker status differs');
...
const expectedIds = ['nar-saga-racecourse-2026-09-19'];
...
assert(records[0].date === START_DATE, `extension date differs: ${records[0].date}`);
...
const output = {
  ...
  adapter_id: 'nar-september-2026-horizon-extension-2026-09-19-reviewed-schedule-promotion-v1',
  country_id: 'japan',
  authority_id: 'nar-local-government-racing',
  source_id: 'nar-monthly-schedule-grid',
  candidate_window: {
  ...
  review: {
    ...
    reviewer: REVIEWER,
    summary: 'Approved one Rank C NAR Saga meeting identity for September 19 from the official September 2026 monthly schedule; no race times, rows, participants, betting, or result data.',
...
console.log(JSON.stringify({
  ...
  publication_rank: 'C',
  detail_status: batch.detail_blockers[0].status,
