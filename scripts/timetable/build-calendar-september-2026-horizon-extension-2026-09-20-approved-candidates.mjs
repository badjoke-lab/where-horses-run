...
const OUTPUT = 'data/candidates/nar-horizon-extension-2026-09-20-approved.json';
const REVIEWED_AT = '2026-08-22T16:58:00Z';
const REVIEWER = 'badjoke-lab';
const START_DATE = '2026-09-20';
const END_DATE_EXCLUSIVE = '2026-09-21';
...
assert(batch.batch_id === 'due-job-plan-2026-08-22-due-japan-nar-coverage-gap-001-run-001', 'NAR review batch identity differs');
assert(batch.generated_at === '2026-08-22T15:58:09.771Z', 'NAR review batch timestamp differs');
...
assert(Array.isArray(batch.scheduled_meetings) && batch.scheduled_meetings.length === 2, 'NAR scheduled meeting count differs');
assert(Array.isArray(batch.schedule_candidates) && batch.schedule_candidates.length === 2, 'NAR schedule candidate count differs');
assert(Array.isArray(batch.detail_candidates) && batch.detail_candidates.length === 0, 'NAR detail candidates must remain empty');
assert(Array.isArray(batch.detail_blockers) && batch.detail_blockers.length === 2, 'NAR detail blocker count differs');
const expectedIds = [
...
const output = {
  ...
  adapter_id: 'nar-september-2026-horizon-extension-2026-09-20-reviewed-schedule-promotion-v1',
  country_id: 'japan',
  authority_id: 'nar-local-government-racing',
  source_id: 'nar-monthly-schedule-grid',
  candidate_window: {
  ...
  review: {
    ...
    reviewer: REVIEWER,
    summary: 'Approved two Rank C NAR meeting identities for September 20 from the official September 2026 monthly schedule; no race times, rows, participants, betting, or result data.',
