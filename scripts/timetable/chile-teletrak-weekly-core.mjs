...
function decodeEntities(value) {
  return value.replace(/&nbsp;|&#160;/gi, ' ').replace(/&amp;/gi, '&').replace(/&aacute;/gi, 'á').replace(/&eacute;/gi, 'é').replace(/&iacute;/gi, 'í').replace(/&oacute;/gi, 'ó').replace(/&uacute;/gi, 'ú').replace(/&ntilde;/gi, 'ñ').replace(/&#(\d+);/g, (_, code) => String.fromCodePoint(Number(code)));
}

function htmlToText(html) {
  const withImageLabels = html.replace(/<img\b[^>]*\balt\s*=\s*(["'])(.*?)\1[^>]*>/gi, (_, _quote, alt) => ` ${alt} `);
...
export function parseChileTeletrakWeeklyHtml(html, { referenceDate, startDate = null, endDateExclusive = null } = {}) {
  ...
  for (let index = 0; index < dayMatches.length; index += 1) {
    ...
    for (const venue of matchedVenues) {
      ...
      seen.add(key);
      records.push({ date, venue_label: venue.canonical_label, racecourse_id: venue.racecourse_id });
    }
  }
...
export function buildChileTeletrakCandidate({ html, checkedAt, startDate, endDateExclusive }) {
  ...
  return {
    candidate: {
      ...
      review: { status: 'needs_review', reviewed_at: null, reviewer: null, summary: 'Chile Teletrak weekly observations use centrally derived best-available rank. The weekly homepage currently proves domestic meeting identity; richer official programme detail is a separate enrichment path.', promotion_target: null },
    },
