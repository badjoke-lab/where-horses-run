...
import { discoverBaneiConfirmedNonRunning } from './banei-non-running-discovery.mjs';
...
try {
  discovery = await discoverBaneiConfirmedNonRunning({ dates, checkedAt: generatedAt });
} catch (error) {
  discovery = {
    records: [],
    diagnostics: [{ source_url: 'https://www.banei-keiba.or.jp/tp_list.php', status: 'discovery_failed', error: String(error?.message ?? error) }],
  };
}
...
const failures = discovery.diagnostics.filter((row) => ['archive_fetch_failed', 'discovery_failed'].includes(row.status));
const artifact = {
  schema_version: 'banei-non-running-evidence-v1',
...
console.log(JSON.stringify({
  ...
  archive_failure_count: failures.length,
}));
