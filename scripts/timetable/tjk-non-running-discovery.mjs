...
export const TJK_NEWS_QUERY_URL = 'https://www.tjk.org/TR/YarisSever/Query/Page/Haberler?QueryParameter_InTurkish=1&QueryParameter_IsYarisSever=1';
...
export async function discoverTjkConfirmedNonRunning({
  ...
  const allowed = new Set(meetingDates);
  const records = [];
  const diagnostics = [];

  try {
    const page = await fetchHtml(TJK_NEWS_QUERY_URL, fetchImpl);
