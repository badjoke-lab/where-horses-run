...
const INPUT = 'data/generated/timetable/horizon-recovery-2026-08-17/nar-coverage-gap-batch.json';
const OUTPUT = 'data/candidates/nar-horizon-recovery-2026-09-15-approved.json';
const REVIEWED_AT = '2026-08-17T16:10:00Z';
const REVIEWER = 'badjoke-lab';
const START_DATE = '2026-09-07';
const END_DATE_EXCLUSIVE = '2026-09-16';
...
assert(batch.schema_version === 'nar-incremental-batch-v2', 'NAR recovery batch schema differs');
assert(batch.review?.status === 'needs_review', 'NAR recovery batch must begin at needs_review');
assert(batch.review?.promotion_eligible === false, 'source NAR recovery batch must remain promotion ineligible');
assert(batch.review?.canonical_write === 'disabled', 'source NAR recovery batch canonical write must remain disabled');
assert(batch.review?.public_write === 'disabled', 'source NAR recovery batch public write must remain disabled');
...
assert(Array.isArray(batch.scheduled_meetings) && batch.scheduled_meetings.length > 0, 'NAR scheduled meetings are missing');
...
assert(records.length === batch.scheduled_meetings.length, 'reviewed recovery must cover the complete requested NAR schedule window');
assert(records[0].date === START_DATE, `recovery first date differs: ${records[0].date}`);
assert(records.at(-1).date === '2026-09-15', `recovery final date differs: ${records.at(-1).date}`);
...
const output = {
  ...
  adapter_id: 'nar-september-2026-horizon-recovery-reviewed-schedule-promotion-v1',
  country_id: 'japan',
  authority_id: 'nar-local-government-racing',
  source_id: 'nar-monthly-schedule-grid',
  candidate_window: {
