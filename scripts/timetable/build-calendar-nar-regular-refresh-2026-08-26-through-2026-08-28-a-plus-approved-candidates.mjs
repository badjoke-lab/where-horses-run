...
const OUTPUT = 'data/candidates/nar-regular-refresh-2026-08-26-through-2026-08-28-a-plus-approved.json';
const REVIEWED_AT = '2026-08-24T16:15:00Z';
...
const EXPECTED_APPROVED = new Map([
  ['nar-funabashi-racecourse-2026-08-26', { rows: 12, first: '14:35', last: '20:50' }],
  ...
  ['nar-monbetsu-racecourse-2026-08-26', { rows: 12, first: '14:15', last: '20:30' }],
  ['nar-sonoda-racecourse-2026-08-26', { rows: 12, first: '10:40', last: '16:30' }],
...
const records = APPROVED_IDS.map((id) => {
  ...
  const record = {
    ...
    notes: 'Approved from the pinned August 24 NAR regular-refresh complete-detail set after exact current-canonical comparison. Only public-safe A+ timetable fields are promoted.',
  };
...
const output = {
  ...
  adapter_id: 'nar-regular-refresh-2026-08-26-through-2026-08-28-reviewed-detail-promotion-v1',
  country_id: 'japan',
  ...
  review: {
    ...
    reviewer: REVIEWER,
    summary: 'Approved exactly seven C-to-A+ NAR detail promotions from the pinned August 24 regular-refresh batch. Six complete-detail records were verified exact no-ops against current canonical A+; 35 schedule-only records remain unapproved.',
    promotion_target: PROMOTION_TARGET,
  },
};

assert(records.length === 7, `expected 7 reviewed delta records, got ${records.length}`);
assert(records.reduce((sum, record) => sum + record.timetable_rows.length, 0) === 84, 'reviewed race-row total differs');
