...
const { candidate, diagnostics } = buildChileTeletrakCandidate({
  html,
...
if (diagnostics.unknown_venues.length > 0) {
  throw new Error(`Chile Teletrak unknown domestic meeting card(s): ${JSON.stringify(diagnostics.unknown_venues)}`);
}
if (diagnostics.parse_failures.length > 0) {
  throw new Error(`Chile Teletrak parse failure(s): ${JSON.stringify(diagnostics.parse_failures)}`);
}
...
const artifact = {
  schema_version: 'chile-teletrak-official-window-candidates-v1',
  ...
  discovery: {
    method: 'official_teletrak_weekly_homepage',
  ...
  window: {
    ...
    coverage_note: 'Teletrak exposes a rolling weekly domestic meeting view. Only source-visible meetings are emitted; absence beyond that visible horizon is not treated as proof of no meeting.',
  },
