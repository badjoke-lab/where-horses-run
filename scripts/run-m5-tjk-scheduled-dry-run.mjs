...
function runtimeIdentity(runAt) {
  ...
  return {
    runId: process.env.GITHUB_RUN_ID || `local-${compact}`,
    attempt: Number(process.env.GITHUB_RUN_ATTEMPT || '1'),
    adapterVersion: process.env.GITHUB_SHA || 'local-uncommitted',
  };
}
...
export async function executeTjkScheduledDryRun({
  runAt,
  outputDir,
  fetchImpl = fetch,
  baseline = DEFAULT_BASELINE,
  baselineDetails = DEFAULT_DETAILS,
  clock = () => new Date(),
} = {}) {
  ...
  try {
    ...
    const log = buildScheduledCandidateRunLog({
      ...baseLogInput({ runAt, runId, attempt, adapterVersion, startedAt, completedAt, windowStart: today, windowEnd: today }),
      status,
      error_code: status === 'source_error' ? 'tjk-source-fetch-failed' : 'tjk-entry-route-rejected',
      error_message: safeErrorMessage(error),
    });
    const runLogPath = writeRunLog(normalizedOutputDir, log);
    return { ok: false, status, runLogPath, candidatePath: null, diffPath: null };
  }
  ...
  try {
    ...
    const log = buildScheduledCandidateRunLog({
      ...baseLogInput({ runAt, runId, attempt, adapterVersion, startedAt, completedAt, windowStart: today, windowEnd: today }),
      status: 'route_or_provenance_error',
      error_code: 'tjk-source-batch-validation-failed',
      error_message: safeErrorMessage(error),
    });
    const runLogPath = writeRunLog(normalizedOutputDir, log);
    return { ok: false, status: 'route_or_provenance_error', runLogPath, candidatePath: null, diffPath: null };
  }
  ...
  try {
    ...
    const log = buildScheduledCandidateRunLog({
      ...baseLogInput({ runAt, runId, attempt, adapterVersion, startedAt, completedAt, windowStart: today, windowEnd: today }),
      status: 'candidate_validation_error',
      error_code: 'tjk-rank-c-candidate-validation-failed',
      error_message: safeErrorMessage(error),
    });
    const runLogPath = writeRunLog(normalizedOutputDir, log);
    return { ok: false, status: 'candidate_validation_error', runLogPath, candidatePath: null, diffPath: null };
  }
  ...
  const log = buildScheduledCandidateRunLog({
    ...baseLogInput({
      ...
      startedAt,
      completedAt,
      windowStart: candidate.candidate_window.start_date,
      windowEnd,
    }),
    status,
