...
function fingerprints(key, html) {
  ...
  if(key==='nztrPartial') return {...common,match:s.includes('awapuni abandonment')&&s.includes('following a slip in the first race')&&s.includes('abandonment at awapuni')};
  if(key==='hrnzCalledOff') return {...common,match:s.includes('thursday meeting called off')&&s.includes('28 horses remained')&&s.includes('cancel the meeting')};
