...
const detailData = json('data/generated/timetable/public/meeting-details.json');
...
const julyDetails = detailData.details.filter((detail) => julyIds.has(detail.meeting_id));
const allJraDetails = detailData.details.filter((detail) => detail.country_id === 'japan' && detail.authority_id === 'jra');
...
for (const detail of allJraDetails) {
  if (!['A', 'A+'].includes(detail.effective_public_rank)) fail(`${detail.meeting_id}: invalid rank`);
  if (julyIds.has(detail.meeting_id) && detail.effective_public_rank !== 'A+') fail(`${detail.meeting_id}: July rank`);
  const fields = detail.effective_public_rank === 'A+' ? aPlusFields : aFields;
  for (const row of detail.timetable_rows) {
    for (const key of Object.keys(row)) if (!fields.has(key)) fail(`${detail.meeting_id}: unexpected ${key}`);
    for (const key of fields) if (!(key in row) || row[key] === null || row[key] === '') fail(`${detail.meeting_id}: missing ${key}`);
  }
}
