...
for (const marker of [
  ...
  '.main-content .timetable-date-group .meeting-row',
  '.main-content .timetable-date-group .meeting-row__identity',
...
assert.match(mobileWorkflow, /const|run-v1-mobile-qa-browser\.mjs/);
...
console.log('- 720px timetable rows improve scan order without changing data markup');
console.log('- metadata uses two columns and collapses to one column at 420px');
console.log('- meeting detail and official-source actions remain at least 44px tall');
console.log('- no timetable rank or A+ data fields are added');
