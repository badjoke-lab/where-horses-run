...
page.on('request', (request) => {
  if (request.method() !== 'POST' || !/sorec-galop\.ma/i.test(request.url())) return;
  const body = request.postData() ?? '';
  const keys = [...new URLSearchParams(body).keys()];
  console.log(JSON.stringify({ type: 'browser_post', url: request.url(), resource_type: request.resourceType(), post_keys: [...new Set(keys)] }));
});
page.on('response', async (response) => {
  const headers = response.headers();
  const contentType = headers['content-type'] ?? '';
  const disposition = headers['content-disposition'] ?? '';
  if ((response.request().method() === 'POST' && /sorec-galop\.ma/i.test(response.url())) || /pdf|octet-stream/i.test(contentType) || disposition) {
    console.log(JSON.stringify({ type: 'browser_response', status: response.status(), url: response.url(), method: response.request().method(), content_type: contentType, content_disposition: disposition }));
  }
});
...
console.log(JSON.stringify({ type: 'row_count', count: await rows.count() }));
let target = null;
for (let i = 0; i < await rows.count(); i += 1) {
  ...
  if (text.includes('10/09/2026') && /Meknes/i.test(text)) {
    target = row;
    console.log(JSON.stringify({ type: 'target_row', index: i, text, visible: await row.isVisible().catch(() => false) }));
    break;
  }
}
...
console.log(JSON.stringify({ type: 'button', id: await button.getAttribute('id'), visible: await button.isVisible().catch(() => false) }));
...
if (postResponse) {
  console.log(JSON.stringify({ type: 'clicked_post_response', status: postResponse.status(), url: postResponse.url(), content_type: postResponse.headers()['content-type'] ?? '', content_disposition: postResponse.headers()['content-disposition'] ?? '' }));
}
