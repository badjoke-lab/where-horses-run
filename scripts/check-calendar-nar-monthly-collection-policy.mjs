...
const japanAuditPolicy = read('docs/calendar/japan-full-month-scope-policy.md');
...
if (policy.scope?.month_start !== '2026-07-01' || policy.scope?.month_end !== '2026-07-31') fail('July completion audit must cover the whole July month.');
if (policy.scope?.partial_cutoff_completion_allowed !== false) fail('partial cutoffs must not count as completion.');
if (policy.scope?.racecourses_checked !== 14 || policy.scope?.flat_racecourses !== 14) fail('July audit must check fourteen flat racecourses.');
if (policy.scope?.banei_excluded !== true) fail('Banei must remain outside the NAR flat scope.');

if (policy.preconditions?.all_14_complete_fixture_set_required !== true) fail('July audit collector must require the all-14 fixture set first.');
...
if (policy.success_condition?.full_calendar_month_required !== true) fail('July completion-audit full-calendar-month rule is missing.');
if (policy.success_condition?.all_fourteen_racecourses_classified !== true) fail('all racecourses must be classified for the July audit.');
if (policy.success_condition?.every_scheduled_meeting_has_explicit_state !== true) fail('every scheduled meeting must have an explicit July audit state.');
...
if (policy.promotion_boundary?.candidate_promotion_eligible !== false) fail('audit candidates must not be promotion-eligible before review.');
...
if (policy.promotion_boundary?.partial_promotions_do_not_complete_month !== true) fail('partial promotions must not complete the July audit claim.');
...
for (const phrase of [
  ...
  if (!incremental.includes(phrase)) fail(`incremental contract missing: ${phrase}`);
}
for (const phrase of [
...
for (const phrase of [
  'applies only when making an explicit July completion claim',
  'incomplete July audit must not block valid reviewed partial batches',
  'Completion claim conditions',
]) {
  if (!japanAuditPolicy.includes(phrase)) fail(`Japan completion-audit policy missing: ${phrase}`);
}
...
for (const marker of ['2026-07-01 through 2026-07-31 inclusive', 'It does not require every July meeting to be available', 'Banei-specific']) {
  if (!baneiPlan.includes(marker)) fail(`Banei plan missing marker: ${marker}`);
}
...
console.log('JULY_AUDIT_BOUNDARY: 2026-07-01..2026-07-31');
console.log('RACECOURSES_CLASSIFIED_FOR_AUDIT: 14');
console.log('ORDINARY_PARTIAL_BATCHES_BLOCKED_BY_AUDIT: false');
