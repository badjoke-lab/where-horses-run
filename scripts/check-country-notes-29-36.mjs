...
const expected = [
  ['29', 'united-kingdom', 'United Kingdom', 'C'],
  ['30', 'united-states', 'United States', 'C'],
  ['31', 'australia', 'Australia', 'C'],
  ['32', 'ireland', 'Ireland', 'A'],
  ['33', 'france', 'France', 'A'],
  ['34', 'canada', 'Canada', 'C'],
  ['35', 'saudi-arabia', 'Saudi Arabia', 'A'],
  ['36', 'india', 'India', 'C']
];
...
for (const [deliveryNo, slug, country, ceiling] of expected) {
  ...
  if (!note.includes('| Evidence cutoff | 2026-06-20 |')) fail(`${slug} evidence cutoff must be 2026-06-20`);
  if (!note.includes(`| Public display ceiling | ${ceiling} |`)) fail(`${slug} ceiling must be ${ceiling}`);
  if (!note.includes('[VERIFIED]')) fail(`${slug} requires VERIFIED labels`);
...
for (const [deliveryNo, slug] of expected) {
  ...
  if (row.evidence_reviewed_at !== '2026-06-20') fail(`${slug} evidence_reviewed_at must be 2026-06-20`);
}
...
console.log('COUNTRY_NOTES_29_36_VALID');
