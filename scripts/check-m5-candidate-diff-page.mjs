...
const changed = diff.rows.find((row) => row.key === 'meeting:m1');
assert.equal(changed.state, 'changed');
assert.deepEqual(changed.changed_fields, ['timetable_rows']);
const unchanged = diff.rows.find((row) => row.key === 'meeting:m2');
assert.equal(unchanged.state, 'unchanged');
const candidateOnly = diff.rows.find((row) => row.key === 'meeting:m4');
assert.equal(candidateOnly.state, 'candidate_only');
...
assert.doesNotMatch(html, /deploy/i);

assert.throws(
  () => buildCandidateDiff({ ...candidate, records: [...candidate.records, { ...candidate.records[0] }] }, baseline, details),
  /duplicate candidate identity/
);

assert.throws(
  () => buildCandidateDiff({ ...candidate, records: [{ ...candidate.records[0], odds: ['2.0'] }] }, baseline, details),
  /prohibited from candidate review output/
);

assert.throws(
  () => buildCandidateDiff({ ...candidate, records: [{ ...candidate.records[0], source: { official_url: 'https://example.test/meeting?access_token=secret' } }] }, baseline, details),
  /sensitive query parameter/
);

assert.throws(
  () => buildCandidateDiff({ ...candidate, records: [{ ...candidate.records[0], country_id: 'japan' }] }, baseline, details),
  /one country\/authority review partition/
);

assert.throws(
  () => buildCandidateDiff({ ...candidate, schema_version: 'unknown-v1' }, baseline, details),
  /unsupported candidate schema/
);

const escaped = buildCandidateDiff({
  ...candidate,
  records: [{ ...candidate.records[2], meeting_id: 'm<script>', source: { official_url: 'https://example.test/izmir' } }]
}, baseline, details);
