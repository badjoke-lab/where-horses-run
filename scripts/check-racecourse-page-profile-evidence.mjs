...
if (audit.implemented?.reviewed_japan_records !== 13 || audit.implemented?.identity_only_records !== 0 || audit.implemented?.no_profile_records !== 0 || audit.implemented?.complete_core_profiles !== 9) fail('implemented counts differ');
if (audit.implemented?.field_counts?.city !== 36 || audit.implemented?.field_counts?.region !== 33 || audit.implemented?.field_counts?.racing_types !== 36 || audit.implemented?.field_counts?.surfaces !== 36 || audit.implemented?.field_counts?.direction !== 30 || audit.implemented?.field_counts?.course_profile !== 24 || audit.implemented?.field_counts?.distance_profile !== 11) fail('implemented field counts differ');
...
const narExpected = new Map([
  ['monbetsu-racecourse', ['Hidaka', 'Hokkaido', 'right-handed', 1600, null, 330, false, '01']],
  ['morioka-racecourse', ['Morioka', 'Iwate', 'left-handed', 1600, 1400, 300, false, '02']],
  ['urawa-racecourse', ['Saitama', 'Saitama', 'left-handed', 1200, null, 220, false, '05']],
  ['funabashi-racecourse', ['Funabashi', 'Chiba', 'left-handed', 1400, null, 300, false, '06']],
  ['oi-racecourse', ['Shinagawa', 'Tokyo', 'right-handed', 1600, null, 386, true, '07']],
  ...
  ['kasamatsu-racecourse', ['Kasamatsu', 'Gifu', 'right-handed', 1100, null, 238, false, '10']],
...
for (const record of evidence.records) {
  ...
  if (!record.course_profile || record.course_profile.dirt_circumference_m === null || record.course_profile.home_straight_m === null) fail(`${record.id}: reviewed course metrics missing`);
  ...
  if (!Array.isArray(record.official_links) || record.official_links.length < 3) fail(`${record.id}: official source set is incomplete`);
  ...
  if (record.id === 'kokura-racecourse') {
    ...
    if (record.course_profile.turf_circumference_m !== 1615 || record.course_profile.dirt_circumference_m !== 1445 || record.course_profile.home_straight_m !== null) fail('Kokura representative course metrics differ');
    if (JSON.stringify(record.distance_profile?.turf?.known_distances_m) !== JSON.stringify([1615, 1633, 1652])) fail('Kokura turf course lengths differ');
    if (JSON.stringify(record.distance_profile?.dirt?.known_distances_m) !== JSON.stringify([1445])) fail('Kokura dirt course length differs');
    if (JSON.stringify(record.distance_profile?.jump?.known_distances_m) !== JSON.stringify([415, 1724])) fail('Kokura jump course lengths differ');
    ...
    if (!expected) fail(`${record.id}: unexpected NAR evidence record`);
      ...
      if ('distance_profile' in record) fail(`${record.id}: NAR race-distance profile must remain absent`);
      if (!record.official_links.some((link) => link.url === `https://www.keiba.go.jp/guide/${guideNo}/`) || !record.official_links.some((link) => link.url === 'https://www.keiba.go.jp/guide/course.html')) fail(`${record.id}: NAR evidence routes missing`);
    }
  }
}
...
for (const record of evidence.records) {
  ...
  if (!fs.existsSync(enPath) || !fs.existsSync(jaPath)) fail(`${record.id}: rendered bilingual page missing`);
    ...
    if (!en.includes(String(record.course_profile.home_straight_m)) && record.course_profile.home_straight_m !== null) fail(`${record.id}: rendered straight metric missing`);
...
console.log('COMPLETE_CORE_PROFILES: 9');
console.log('NAR_DISTANCE_PROFILES_RETAINED_UNKNOWN: 12');
