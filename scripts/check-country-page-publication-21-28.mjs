...
for (const phrase of prohibitedPublicPhrases) {
  if (component.includes(phrase)) fail(`CountryDetailPage contains internal-only wording: ${phrase}`);
}
...
for (const [, slug] of expected) {
  ...
  for (const [label, html] of [['EN', en], ['JA', ja]]) {
    ...
    for (const phrase of prohibitedPublicPhrases) {
      if (html.includes(phrase)) fail(`${slug} ${label} contains internal-only wording: ${phrase}`);
    }
  }
}
...
console.log('INTERNAL_COPY_GUARD: valid');
