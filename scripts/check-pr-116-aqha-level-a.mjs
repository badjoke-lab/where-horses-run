...
function fail(message) {
  console.error(`[pr-116-aqha] ${message}`);
  process.exit(1);
}
...
const generator = spawnSync('node', ['scripts/generate-pr-116-aqha-level-a.mjs'], {
  cwd: root,
  encoding: 'utf8'
});
...
const data = readJson('data/generated/timetable/pr-116-aqha-level-a.json');
if (data.schema_version !== 'pr-116-aqha-level-a-v0') fail('Unexpected schema_version.');
if (data.mode !== 'fixture_full_quarter_horse_racecard_no_live_fetch') fail('PR-116 must not live fetch.');
...
for (const record of data.records) {
  ...
  if (record.group_id !== 'aqha-quarter-horse') fail(`${record.record_id}: group_id must be aqha-quarter-horse.`);
  if (record.data_level !== 'A') fail(`${record.record_id}: data_level must be A.`);
  if (!record.promotes_from?.startsWith('pr113-us-aqha-')) fail(`${record.record_id}: promotes_from must reference PR-113 AQHA record.`);
...
console.log(`[pr-116-aqha] PASS: ${data.records.length} Level A full quarter horse racecard records.`);
