...
function parsePage(url) {
  ...
  return { url, html, lang, title, description: descriptions.length === 1 ? descriptions[0].content ?? '' : '', descriptionCount: descriptions.length, canonical: canonicals.length === 1 ? canonicals[0].href ?? '' : '', canonicalCount: canonicals.length, alternates, sections, relatedLinks };
}
function verifyHistorical(contract, audit) {
  expect(contract.schema_version === 'methods-data-policy-contract-v1' && contract.work_id === 'WHR-SEO-PUBLIC-CONTENT-V1' && contract.implementation_unit === 'METHODS-DATA-POLICY-01', 'Methods contract identity differs');
  expect(contract.status === 'complete' && contract.reviewed_at === '2026-07-18', 'Methods historical state differs');
  const historicalScope = { public_pages: 771, english_pages: 387, japanese_pages: 384, methods_pages: 2, languages: 2, sections_per_page: 9, visible_sections: 18, paragraphs_per_page: 18, visible_paragraphs: 36, about_page_method_links: 2, related_page_links: 8 };
  expect(exact(contract.scope, historicalScope), 'Methods historical scope differs');
  expect(contract.public_boundary.revenue_or_monetization_discussion_allowed === false && contract.public_boundary.source_specific_internal_risk_posture_allowed === false, 'Methods public boundary differs');
  ...
  expect(audit.schema_version === 'methods-data-policy-audit-v1' && audit.status === 'complete', 'Methods historical audit identity differs');
  for (const [key, value] of Object.entries({ public_pages: 771, english_pages: 387, japanese_pages: 384, methods_pages: 2, visible_sections: 18, visible_paragraphs: 36, about_page_method_links: 2, related_page_links: 8 })) expect(audit.verified[key] === value, `Methods historical audit ${key} differs`);
  for (const key of ['missing_methods_routes','canonical_errors','hreflang_errors','title_errors','description_errors','section_count_errors','paragraph_count_errors','empty_section_errors','empty_paragraph_errors','topic_coverage_errors','topic_parity_errors','about_navigation_errors','related_navigation_errors','forbidden_public_topic_errors','temporary_discovery_workflows','contract_errors','output_errors']) expect(audit.verified[key] === 0, `Methods historical audit ${key} differs`);
}
function verifySources() {
  ...
  for (const marker of ["locale: 'en' | 'ja'", "canonicalPath = isJa ? '/ja/methods/' : '/methods/'", 'class="card methods-section"', 'class="card methods-related"']) expect(component.includes(marker), `Methods component marker missing: ${marker}`);
  for (const pattern of [/revenue/i,/monetization/i,/budget/i,/other project/i,/internal circumstances/i,/収益/,/予算/,/他プロジェクト/,/内部事情/]) expect(!pattern.test(component), `Methods component contains forbidden public topic ${pattern}`);
  expect(read(EN_ROUTE_PATH).includes('<MethodsPage locale="en" />') && read(JA_ROUTE_PATH).includes('<MethodsPage locale="ja" />'), 'Methods route wiring differs');
  expect(read(EN_ABOUT_PATH).includes('href="/methods/"') && read(JA_ABOUT_PATH).includes('href="/ja/methods/"'), 'About methods navigation differs');
  const doc = read(DOC_PATH);
  for (const marker of ['METHODS-DATA-POLICY-01','Public pages: 771','Visible sections: 18','Visible paragraphs: 36','SEO-QA-RELEASE-01']) expect(doc.includes(marker), `Methods historical documentation marker missing: ${marker}`);
...
function verifyPage(page, route) {
  expect(page.lang === route.lang && page.title === route.title, `${page.url}: visible metadata differs`);
  ...
  expect(page.sections.flatMap((section) => section.paragraphs).length === 18 && page.sections.flatMap((section) => section.paragraphs).every(Boolean), `${page.url}: paragraph structure differs`);
...
function main() {
  ...
  expect(urls.length >= contract.scope.public_pages, `Methods current sitemap shrank ${urls.length}`);
  const en = parsePage(`${SITE_ORIGIN}/methods/`);
  const ja = parsePage(`${SITE_ORIGIN}/ja/methods/`);
  verifyPage(en, { lang: 'en', title: contract.routes.en.title, description: contract.routes.en.description, relatedLinks: ['/about/', '/faq/', '/disclaimer/', '/sources/'], requiredPhrases: ['Official sources','Reference sources','Review before publication','Dates, times, and timezones','Publication ranks','Last checked','Corrections, reduction, and removal','Excluded information and limitations'] });
  verifyPage(ja, { lang: 'ja', title: contract.routes.ja.title, description: contract.routes.ja.description, relatedLinks: ['/ja/about/', '/ja/faq/', '/ja/disclaimer/', '/ja/sources/'], requiredPhrases: ['公式ソース','参考ソース','レビューから公開まで','日付・時刻・タイムゾーン','公開ランク','確認日と更新','訂正・縮小・削除','掲載しない情報と制限事項'] });
