...
assert.match(list, /hasStaleRecords/);
assert.match(list, /status === 'stale'/);
assert.match(list, /Some checks are stale\. Confirm with official sources\./);
assert.match(list, /古い確認を含みます。公式ソースで確認してください。/);
...
console.log('- compact stale warning renders once per meeting list while row check dates remain visible');
