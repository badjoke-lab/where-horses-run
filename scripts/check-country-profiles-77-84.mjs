...
const expected = [
  ['77','kazakhstan','complete','meeting-date-only'],
  ['78','egypt','partial','official-link-only'],
  ['79','algeria','partial','official-link-only'],
  ['80','iran','partial','official-link-only'],
  ['81','vietnam','complete','meeting-date-only'],
  ['82','bolivia','partial','official-link-only'],
  ['83','guatemala','partial','official-link-only'],
  ['84','honduras','partial','official-link-only']
];

const countries = parse('data/static/country-page-countries-77-84.json');
const sources = parse('data/static/country-page-sources-77-84.json');
...
for (const [deliveryNo, slug, sourceStatus, timePattern] of expected) {
  ...
  if (timePattern === 'official-link-only' && !/(do not add current meeting rows|do not infer a future meeting date|explanatory)/i.test(profile.calendar_guidance_en)) {
    fail(`${slug}: link-only or blocked calendar guidance is insufficient`);
  }
}

const dataText = read('src/lib/data.ts');
for (let deliveryNo = 77; deliveryNo <= 84; deliveryNo += 1) if (!dataText.includes(`countryProfilesV2${deliveryNo}`)) fail(`data.ts must load profile batch ${deliveryNo}`);
for (const token of ['countryPageCountries7784','countryPageSources7784']) if (!dataText.includes(token)) fail(`data.ts must load ${token}`);
...
if (counts.published !== 76 || counts.profile_ready !== 8 || counts.not_started !== 14) fail(`effective programme counts mismatch ${JSON.stringify(counts)}`);

if (errors.length) {
  errors.forEach((error) => console.error(`ERROR: ${error}`));
  process.exit(1);
}
console.log('COUNTRY_PROFILES_77_84_VALID countries=8 sources=8 profiles=8');
console.log('PROGRAMME_COUNTS published=76 profile_ready=8 not_started=14');
