...
assert.equal(workflow.split('node scripts/timetable/run-brazil-jcb-current-meetings-window.mjs').length - 1, 2, 'Brazil JCB current-meetings collector must run in both normal and latest-main rebuild paths');
assert.equal(workflow.split('--artifact=.calendar-unified/brazil-gavea.json').length - 1, 4, 'Brazil Gávea artifact must pass exclusion and apply layers in both execution paths');
assert.equal(workflow.split('--artifact=.calendar-unified/brazil-cristal.json').length - 1, 4, 'Brazil Cristal artifact must pass exclusion and apply layers in both execution paths');
assert.equal(workflow.split('--authority-id=jockey-club-brasileiro').length - 1, 2, 'Brazil Gávea observations must be applied in both execution paths');
...
assert.equal(workflow.split('--timezone=America/Sao_Paulo').length - 1, 4, 'Both routed Brazil systems must bind America/Sao_Paulo in both execution paths');
...
console.log('BRAZIL_JCB_GAVEA_UNIFIED_REFRESH_PATHS: 2');
console.log('BRAZIL_JCB_CRISTAL_UNIFIED_REFRESH_PATHS: 2');
