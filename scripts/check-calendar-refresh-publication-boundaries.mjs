...
assert.equal(
  (workflow.match(/run-saudi-jcsa-official-window\.mjs/g) ?? []).length,
  2,
  'Saudi JCSA must be collected in both normal and latest-main rebuild paths',
);
assert.equal(
  (workflow.match(/--authority-id=jockey-club-of-saudi-arabia/g) ?? []).length,
  2,
  'Saudi JCSA observations must be applied in both normal and latest-main rebuild paths',
);
assert.equal(
  (workflow.match(/--artifact=\.calendar-unified\/saudi-arabia\.json/g) ?? []).length,
  4,
  'Saudi JCSA artifact must pass exclusion and apply layers in both execution paths',
);
assert.equal(
  (workflow.match(/--racing-system-id=saudi-arabia-jcsa-system/g) ?? []).length,
  2,
  'Saudi JCSA apply path must bind the canonical racing system id in both execution paths',
);
assert.equal(
  (workflow.match(/--timezone=Asia\/Riyadh/g) ?? []).length,
  2,
  'Saudi JCSA apply path must bind the Saudi timezone in both execution paths',
);
