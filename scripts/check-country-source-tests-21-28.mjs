...
const expected = [
  ['21', 'hungary', 'Hungary', 'Complete at programme level', 'A', 'A', 'remote_complete'],
  ['22', 'malta', 'Malta', 'Complete at calendar level', 'C', 'C', 'remote_complete'],
  ['23', 'austria', 'Austria', 'Partial', 'A for Krieau; country coverage incomplete', 'C', 'remote_partial'],
  ['24', 'puerto-rico', 'Puerto Rico', 'Complete at programme level', 'A', 'A', 'remote_complete'],
  ['25', 'jamaica', 'Jamaica', 'Complete at programme level', 'A', 'A', 'remote_complete'],
  ['26', 'trinidad-and-tobago', 'Trinidad and Tobago', 'Complete at programme level', 'A', 'A', 'remote_complete'],
  ['27', 'barbados', 'Barbados', 'Complete at programme level', 'A', 'A', 'remote_complete'],
  ['28', 'martinique', 'Martinique', 'Partial', 'C', 'C', 'remote_partial']
];
...
for (const [deliveryNo, slug, country, status, rank, ceiling] of expected) {
  ...
  if (summary.checked_date !== '2026-06-18') fail(`${slug} checked_date must be 2026-06-18`);
...
const indexPath = path.join(root, 'docs/timetable-source-tests/21-28-index.md');
if (!fs.existsSync(indexPath)) fail('missing docs/timetable-source-tests/21-28-index.md');
else {
