...
if (policy.policy_version !== 'calendar-route-runner-policy-2026-07-11') fail('route policy version differs.');
...
if (!exact(detail.evidence_backed_observation_ranks, ['B'])) fail('HKJC detail route evidence ranks differ.');
...
if (!profile) fail('HKJC Registry profile missing.');
  ...
  if (profile.detail_source_id !== null || profile.detail_adapter_id !== null) fail('route policy supplement must not activate Registry detail fields.');
  if (!exact(profile.supported_observation_ranks, ['C'])) fail('route policy supplement must not expand Registry observation ranks.');
}
...
if (validateCollectionJobV1(unsafeGenericDetailJob, registry).length === 0) fail('generic Collection Job unexpectedly selected unregistered HKJC reviewed_import detail route.');
...
for (const file of [schedule.entry_point, detail.entry_point, schedule.evidence_ref, detail.evidence_ref]) {
