...
for (const [label, content] of [
  ...
  requireIncludes(content, 'todayRecords', label);
  requirePattern(content, /record\.date\s*={2,3}\s*currentTimetableDate/, label, 'must match records using the generated/current timetable date');
...
requireIncludes(englishPage, 'Today’s safe timetable', englishPath);
requireIncludes(englishPage, 'No safe timetable records are listed for today yet.', englishPath);
...
requireIncludes(japanesePage, '今日の安全な開催時刻データ', japanesePath);
requireIncludes(japanesePage, '今日の安全な開催時刻レコードはまだありません。', japanesePath);
