...
const outputDir = path.join(process.cwd(), 'artifacts/rendered-preview-77-84');
fs.mkdirSync(outputDir, { recursive: true });
const slugs = ['kazakhstan','egypt','algeria','iran','vietnam','bolivia','guatemala','honduras'];
const representatives = new Set(['kazakhstan','vietnam','guatemala','honduras']);
    ...
    for (const slug of slugs) {
      for (const locale of ['en','ja']) {
        ...
        const result = await page.evaluate(({ locale }) => {
          ...
          const emptyState = locale === 'ja' ? text.includes('これは、この国で競馬がないことを意味しません。') : text.includes('This does not mean there is no racing in this country.');
          return { lang: root.lang, h1: document.querySelectorAll('h1').length, overflow: root.scrollWidth - root.clientWidth, media: document.querySelectorAll('iframe, video').length, externalLinks, forbiddenHeaders, canonical, hreflang, languageSwitch, emptyState, fontFamily: getComputedStyle(document.body).fontFamily };
        }, { locale });
...
const report = { checked_at: new Date().toISOString(), base_url: baseUrl, routes: 16, viewport_checks: pages.length, representative_screenshots: 16, viewports, representatives: [...representatives], errors, pages };
fs.writeFileSync(path.join(outputDir, 'report.json'), `${JSON.stringify(report, null, 2)}\n`);
if (errors.length) { errors.forEach((error) => console.error(`ERROR: ${error}`)); process.exit(1); }
console.log('RENDERED_PREVIEW_77_84_VALID routes=16 viewport_checks=32 screenshots=16 errors=0');
