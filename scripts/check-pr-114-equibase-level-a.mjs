...
function fail(message) {
  console.error(`[pr-114-equibase] ${message}`);
  process.exit(1);
}
...
const generator = spawnSync('node', ['scripts/generate-pr-114-equibase-level-a.mjs'], {
  cwd: root,
  encoding: 'utf8'
});
...
const data = readJson('data/generated/timetable/pr-114-equibase-level-a.json');
if (data.schema_version !== 'pr-114-equibase-level-a-v0') fail('Unexpected schema_version.');
if (data.mode !== 'fixture_full_racecard_no_live_fetch') fail('PR-114 must not live fetch.');
...
for (const record of data.records) {
  ...
  if (record.group_id !== 'equibase-thoroughbred') fail(`${record.record_id}: group_id must be equibase-thoroughbred.`);
  if (record.data_level !== 'A') fail(`${record.record_id}: data_level must be A.`);
  if (!record.promotes_from?.startsWith('pr113-us-equibase-')) fail(`${record.record_id}: promotes_from must reference PR-113 Equibase record.`);
...
console.log(`[pr-114-equibase] PASS: ${data.records.length} Level A full racecard records.`);
