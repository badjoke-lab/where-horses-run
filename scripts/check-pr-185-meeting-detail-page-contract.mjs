...
function stop(message) {
  console.error(`[pr-185-meeting-detail-page-contract] ${message}`);
  process.exit(1);
}
...
const policy = JSON.parse(read('data/generated/timetable/pr-185-meeting-detail-page-contract.json'));
const notes = read('PR-185.md');

if (policy.schema_version !== 'pr-185-meeting-detail-page-contract-v0') stop('Unexpected schema version.');
...
for (const key of [
  'adds_routes',
  'adds_ui',
  'adds_source_specific_parsers',
  'adds_scrapers',
  'adds_runtime_fetching',
  'adds_scheduled_jobs',
  'adds_generated_writeback',
  'adds_racecard_redistribution',
  'adds_video_or_stream_integration'
]) {
...
console.log('[pr-185-meeting-detail-page-contract] PASS');
