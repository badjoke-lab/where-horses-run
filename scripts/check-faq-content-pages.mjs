...
function verifyContract(contract, audit) {
  ...
  expect(contract.status === 'complete' && contract.reviewed_at === '2026-07-18', 'FAQ status or date differs');
  expect(contract.scope_updated_by === 'METHODS-DATA-POLICY-01', 'FAQ scope marker differs');
  expect(contract.site_origin === SITE_ORIGIN, 'FAQ origin differs');
  expect(contract.routes.en.path === '/faq/' && contract.routes.ja.path === '/ja/faq/', 'FAQ route contract differs');
  expect(contract.structured_data_contract.type === 'FAQPage', 'FAQ structured type differs');
  expect(contract.structured_data_contract.questions_per_script === contract.scope.questions_per_page, 'FAQ structured count differs');
  ...
  expect(contract.public_boundary.internal_circumstances_allowed === false, 'FAQ internal boundary differs');
  expect(contract.public_boundary.betting_advice_allowed === false, 'FAQ betting boundary differs');
  ...
  expect(audit.schema_version === 'faq-content-pages-audit-v1' && audit.status === 'complete', 'FAQ audit identity differs');
  expect(audit.scope_updated_by === contract.scope_updated_by, 'FAQ audit scope marker differs');
  for (const key of ['public_pages','english_pages','japanese_pages','faq_pages','visible_questions','visible_answers','faqpage_scripts','structured_questions','structured_answers','home_page_faq_links','related_page_links']) {
    expect(audit.verified[key] === contract.scope[key], `FAQ audit ${key} differs`);
  }
  for (const key of ['missing_faq_routes','canonical_errors','hreflang_errors','title_errors','description_errors','missing_questions','missing_answers','duplicate_questions','structured_data_errors','visible_structured_mismatches','webpage_relation_errors','home_navigation_errors','related_navigation_errors','forbidden_public_topic_errors','temporary_discovery_workflows','contract_errors','output_errors']) {
...
function verifySources(contract) {
  ...
  for (const marker of ["locale: 'en' | 'ja'", 'data-faq-structured-data="faq-page-v1"', "'@type': 'FAQPage'", "'@type': 'Question'", "'@type': 'Answer'", 'canonicalPath={canonicalPath}', 'alternatePath={alternatePath}']) {
  ...
  for (const pattern of [/revenue/i,/monetization/i,/budget/i,/other project/i,/internal circumstances/i,/収益/,/予算/,/他プロジェクト/,/内部事情/]) {
    expect(!pattern.test(component), `FAQ component contains forbidden public topic ${pattern}`);
  }
  expect(read(EN_ROUTE_PATH).includes('<FaqPage locale="en" />'), 'English FAQ route differs');
  expect(read(JA_ROUTE_PATH).includes('<FaqPage locale="ja" />'), 'Japanese FAQ route differs');
  expect(read(EN_HOME_PATH).includes('href="/faq/"'), 'English home FAQ link is missing');
  expect(read(JA_HOME_PATH).includes('href="/ja/faq/"'), 'Japanese home FAQ link is missing');
  const doc = read(DOC_PATH);
  for (const marker of ['FAQ-CONTENT-PAGES-01', `Public pages: ${contract.scope.public_pages}`, 'Visible questions: 24', 'Structured questions: 24', 'METHODS-DATA-POLICY-01']) {
...
function verifyPage(page, route, contract) {
  ...
  expect(page.questions.length === contract.scope.questions_per_page, `${page.url}: visible question count differs ${page.questions.length}`);
  expect(page.questions.every((item) => item.question && item.answer), `${page.url}: empty question or answer`);
  ...
  let structured;
  try { structured = JSON.parse(page.faqScripts[0][2]); } catch (error) { throw new Error(`${page.url}: invalid FAQ JSON-LD: ${error.message}`); }
  expect(structured['@context'] === 'https://schema.org' && structured['@type'] === 'FAQPage', `${page.url}: FAQ JSON-LD identity differs`);
  expect(structured['@id'] === `${page.url}#faq` && structured.url === page.url, `${page.url}: FAQ JSON-LD URL differs`);
  expect(structured.name === page.title && structured.description === page.description && structured.inLanguage === page.lang, `${page.url}: FAQ JSON-LD page fields differ`);
...
function main() {
  ...
  expect(urls.length === contract.scope.public_pages, `FAQ sitemap count differs ${urls.length}`);
  expect(urls.includes(`${SITE_ORIGIN}/faq/`) && urls.includes(`${SITE_ORIGIN}/ja/faq/`), 'FAQ routes are missing from sitemap');
  verifyPage(parsePage(`${SITE_ORIGIN}/faq/`), {
  ...
  verifyPage(parsePage(`${SITE_ORIGIN}/ja/faq/`), {
