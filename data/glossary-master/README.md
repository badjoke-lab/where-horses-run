...
`GLOSSARY-MASTER-006` wave 10 is now in progress. The current ERA Arabic Rules provide a directly authority-backed Arabic/original-script layer for six material existing Concepts, increasing source-verified local labels to **259** across **6 languages / 5 represented scripts**. The search layer remains **598 reviewed non-canonical query records** and the no-direct-equivalent audit remains **56 Concepts**. The current UAE Arabic representation gap is closed, but broader worldwide jurisdiction/language and search-intent sufficiency gaps keep MASTER-006 at **HOLD**.
